...
if __name__ == "__main__":
    x = np.load('./tmp/experts/expert_MountainCar-v0.npz', allow_pickle=True)

    X_embedded = x['obs']
    # X_embedded = TSNE(n_components=2, learning_rate='auto', init='random', n_jobs=4, verbose=True).fit_transform(x['obs'])
    # np.save('./tmp/experts/embedded_Antv3', X_embedded)
    # X_embedded = np.load('./tmp/experts/embedded_antv3.npy', allow_pickle=True)

    rewards = defaultdict(int)
    for reward in x['episode_returns']:
        rewards[reward] += 1

    idx = np.where(x['episode_returns'] >= -110)[0]
        
    begginings = np.where(x['episode_starts'] == True)[0][idx]
